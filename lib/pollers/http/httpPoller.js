...
HttpPoller.prototype.poll = function() {
  ...
  if (this.target['method'] == "POST")
    this.request = client.post(this.target['href']).type("json").send(this.target['body']).end(function(err, res){
      console.log("in superagent.POST")
      if (err){
        _self.onErrorCallback.call(_self, err);
        return;
      }

      _self.onResponseCallback.call(_self, res);
    });

  if (this.target['method'] == "GET")
    this.request = client.get(this.target['href']).end(function(err, res){
      if (err){
        _self.onErrorCallback.call(_self, err);
        return;
      }
